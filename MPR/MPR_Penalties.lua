...
function MPR_Penalties:Initialize()
	...
	MPR_Penalties:NewSP("|cFF9370DB[Vengeful Blast]|r","VB",10,-30)
...
function MPR_Penalties:HandleClick_Deduct(n)
	...
	local NewNet = MPR_Penalties:DeductDKP(MPR.DataPenalties[i].Player,MPR.Settings["PENALTIES_"..MPR.DataPenalties[i].Spell.."_AMOUNT"])
	if NewNet ~= nil then
		MPR_Penalties:AnnounceDeduction(Player,Amount,NewNet,MPR.DataPenalties[i].Spell)
	end
	MPR_Penalties:RefreshList()
end
